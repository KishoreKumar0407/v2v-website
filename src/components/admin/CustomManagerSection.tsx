...
/** Inline custom manager card — same UX as Experiment Manager (Add Project form + list in dashboard). */
export default function CustomManagerSection({ manager: initial, user, onDeleted }: Props) {
    ...
    const saveRecord = async (values: Record<string, string>) => {
        ...
        try {
            ...
            toast.showToast(editingRecord ? '✅ Project updated' : '✅ Project added', 'success');
            setShowForm(false);
            setEditingRecord(null);
            await load();
        } catch (e) {
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div>
                            <CardTitle className="flex items-center gap-2 text-cyan-400">
                                ...
                                {manager.name}
                                <span className="text-xs bg-cyan-500/20 text-cyan-300 px-2 py-0.5 rounded-full border border-cyan-500/30 font-normal">Custom Manager</span>
                            </CardTitle>
                            <CardDescription className="mt-2">
                                ...
                                <span className="block">{manager.description || `Manage ${manager.name} projects and information.`}</span>
                            </CardDescription>
                        </div>
