...
/** Experiment Manager — same UX as CustomManagerSection but labeled for experiments. */
export default function ExperimentManager({ manager: initial, user, onDeleted }: Props) {
    ...
    const saveRecord = async (values: Record<string, string>) => {
        ...
        try {
            ...
            toast.showToast(editingRecord ? '✅ Experiment updated' : '✅ Experiment added', 'success');
            setShowForm(false);
            setEditingRecord(null);
            await load();
        } catch (e) {
    ...
    const deleteRecord = async (record: ManagerRecord) => {
        ...
        try {
            ...
            toast.showToast('✅ Experiment deleted', 'success');
            await load();
        } catch {
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div>
                            <CardTitle className="flex items-center gap-2 text-emerald-400">
                                ...
                                {manager.name}
                                <span className="text-xs bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded-full border border-emerald-500/30 font-normal">Experiment Manager</span>
                            </CardTitle>
                            <CardDescription className="mt-2">
                                ...
                                <span className="block">{manager.description || `Manage ${manager.name} experiments and information.`}</span>
                            </CardDescription>
                        </div>
                        <div className="flex flex-wrap gap-2 shrink-0">
                            <Button onClick={openNew} className="bg-emerald-600 hover:bg-emerald-500 text-white">
                                <Plus className="w-4 h-4 mr-2" /> Add Experiment
                            </Button>
                            {isMainAdmin && (
                                <Button
                                    variant="outline"
                        ...
                        <>
                            {showForm && (
                                <ManagerRecordForm
                                    ...
                                    title={editingRecord ? 'Edit Experiment' : 'New Experiment'}
                                    submitText={editingRecord ? 'Update Experiment' : 'Add Experiment'}
